...
class Replay_Buffer(object):
    ...
    def sample(self, phi, num_experiences=None, separate_out_data_types=True):
        """Draws a random sample of experience from the replay buffer"""
        experiences = self.pick_experiences(num_experiences)
        if separate_out_data_types:
            states, actions, rewards, next_states, dones = self.separate_out_data_types(experiences, phi)
            return states, actions, rewards, next_states, dones
        else:
            return experiences
            
    def separate_out_data_types(self, experiences, phi):
        ...
        # for e in experiences:
        #     if e is not None:
        #         for x in e.next_state:
        #             next_states.append(x)

        states = phi(states, True)
        next_states = phi(next_states, True)
