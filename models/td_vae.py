# From: https://github.com/xqding/TD-VAE
...
class PreProcess(nn.Module):
    """ The pre-process layer for set of points

    """
    def __init__(self, processed_x_size):
    ...
    def forward(self, input):
        ...
        xb, matrix3x3, matrix64x64 = self.ptnet(input)
        return xb
...
class TD_VAE(nn.Module):
    ...
    def __init__(self, x_size, processed_x_size, b_size, z_size):
        ...
        self.process_x = PreProcess(self.processed_x_size)
    ...
    def forward(self, images):        
        ...
        self.x = images
        ## pre-precess image x
        self.processed_x = self.process_x(self.x).unsqueeze(0)
        print(self.processed_x.shape)


        ## aggregate the belief b
        self.b, (h_n, c_n) = self.lstm(self.processed_x)
        print(self.b.shape)
        z = self.l2_b_to_z(self.b[:, :, :])
...
model = TD_VAE(90, 256, 64, 32)
# This TD-VAE takes in points of shape [sequence, ]
model(torch.rand(size=(10,2,100)))

print('next')
model(torch.rand(size=(10,2,150)))