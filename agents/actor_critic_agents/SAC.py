...
class SAC(Base_Agent):
    ...
    def __init__(self, config):
        ...
        for exp in pre_experiences:
            ...
            self.save_experience(experience=experience)
    ...
    def reset_game(self, is_eval=False):
        ...
        Base_Agent.reset_game(self, is_eval=is_eval)
    ...
    def produce_action_and_action_info(self, state):
        ...
        # mean_squashed = torch.where(idxs, mean_squashed, torch.tensor(0.0).cuda())
        # action = torch.where(idxs, action, torch.tensor(0.0).cuda())
    ...
    def learn(self):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch = self.sample_experiences(self.phi)
    ...
    def sample_experiences(self, phi):
        return  self.memory.sample(self.phi)
